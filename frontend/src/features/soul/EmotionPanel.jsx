...
// Bar color: monochrome ink ladder — stronger values read darker.
function barColor(value) {
  if (value >= 75) return 'bg-[var(--accent)]';
  if (value >= 55) return 'bg-[var(--text-secondary)]';
  if (value >= 35) return 'bg-[var(--text-muted)]';
...
export default function EmotionPanel({ refreshTrigger = 0 }) {
  ...
  if (loading) {
    return <div className="pt-4 text-sm text-[var(--text-muted)] flex items-center gap-2"><RefreshCw size={14} className="animate-spin" /> {t('settings.emotion.loading')}</div>;
  }

  if (error || !values) {
    return (
      <div className="pt-4 space-y-2">
        <p className="text-xs text-[var(--text-muted)]">{t('settings.emotion.no_target')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 pt-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-[var(--text-muted)]">{t('settings.emotion.description')}</p>
        <button onClick={load} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]" title={t('settings.emotion.refresh')}>
          <RefreshCw size={13} />
        </button>
      </div>
