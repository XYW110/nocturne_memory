...
const GlossaryPopup = ({ keyword, nodes, position, onClose, onNavigate }) => {
  ...
  return createPortal(
    <div
      ref={popupRef}
      className="fixed z-[100] w-72 bg-nocturne-bg-secondary border border-amber-800/40 rounded-xl shadow-2xl shadow-black/60 overflow-hidden flex flex-col"
      style={{ 
        left: position.x, 
        ...(position.isAbove 
    ...
    >
      <div className="px-3 py-2 border-b border-[var(--color-border-strong)] flex items-center gap-2 flex-shrink-0">
        <BookOpen size={12} className="text-amber-400" />
        <span className="text-xs font-semibold text-amber-300">{keyword}</span>
        <button onClick={onClose} className="ml-auto text-nocturne-text-muted hover:text-nocturne-text-secondary transition-colors">
          <X size={12} />
        </button>
      </div>
        ...
        {nodes.map((node, i) => {
          ...
          <button
            ...
            className={clsx(
              ...
              isUnlinked ? "cursor-default opacity-80 bg-nocturne-bg-tertiary/40" : "hover:bg-nocturne-bg-tertiary/60 cursor-pointer"
            )}
          >
            <div className="flex items-center justify-between gap-2">
              <code className={clsx(
                "text-[11px] font-mono block truncate flex-1",
                isUnlinked ? "text-nocturne-text-muted" : "text-indigo-400/80 group-hover:text-indigo-300"
              )}>
                {node.uri}
              </code>
              {isUnlinked && (
            ...
            {node.content_snippet && (
              <p className="text-[10px] text-nocturne-text-muted mt-0.5 line-clamp-2 leading-snug">
                {node.content_snippet}
              </p>
            )}
          </button>
...
const GlossaryHighlighter = ({ content, glossary, currentNodeUuid, onNavigate }) => {
  ...
  if (matches.length === 0) {
    return <pre className="whitespace-pre-wrap font-serif text-nocturne-text-primary leading-7">{content}</pre>;
  }
  ...
  return (
    <div ref={containerRef} className="relative">
      <pre className="whitespace-pre-wrap font-serif text-nocturne-text-primary leading-7">
        {parts.map((part, i) =>
          part.isMatch ? (
            <span
              key={i}
