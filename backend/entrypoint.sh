...
# Auto-fix database_url if it points to old paths
_fix_config() {
    if [ -f "$PERSISTENT_DIR/config.json" ]; then
        # Check if database_url is correct (should point to /app/persistent/data/)
        if grep -q '///app/data/' "$PERSISTENT_DIR/config.json" || \
           ...
           grep -q 'sqlite+aiosqlite:///d:' "$PERSISTENT_DIR/config.json"; then
            ...
            # Generate new API token only if needed
            NEW_API_TOKEN=$(python -c "import secrets; print(secrets.token_urlsafe(32))")
