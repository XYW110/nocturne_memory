...
Idempotent: paths that already exist are skipped, never overwritten.
"""
...
class TemplateLoader:
    ...
    async def apply_template(
        ...
        async with self.db.session() as session:
            for node in all_nodes:
                await self._apply_node(session, node, values, namespace, result)
    ...
    async def _apply_node(
        ...
        # Idempotency: skip if the path already exists.
    ...
    async def _configure_boot_uris(self, nodes: list[dict], namespace: str) -> None:
        """Append top-level template URIs to the active preset's boot list."""
        from db import get_preset_service

        root_uris = [
            f"{n.get('domain', 'core')}://{n['path']}"
            for n in nodes
            if "/" not in n.get("path", "")
        ]
        if not root_uris:
            return
